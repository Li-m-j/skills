#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""paper_search_client.py — se_paper_search 检索客户端（v0.1.0 · 零第三方依赖）

qm_paper_search（化学）/ mbai_paper_search（医学）的第三个领域孪生版：
统计科学 / 计量经济学 / 经济学 文献检索。

职责：
    1. 多源检索：arXiv（econ.EM / stat.* 预印本主渠道）→ OpenAlex（覆盖主链）
       → Semantic Scholar（引用 / TLDR）→ Crossref（元数据权威）
    2. 规范化 + 合并：DOI / arXiv ID / 标题去重，字段级补全，**缺则 N/A，绝不编造**
    3. 方案 H 宽召回：`--concept-pattern` regex 二次方向过滤（不额外发请求）
    4. seen_papers.json 读写：topic_id 前缀 + 跨 pool 合并去重
    5. 导出 SKILL.md §6.2 规定的 Markdown 名录（元信息块 + 速览 + 详细条目 + 4 引用格式）
    6. `--verify`：交付前复用 `validate_output.py` 做 DOI + arXiv ID 反查

依赖：仅 Python 3.8+ 标准库（urllib）。Windows / Linux / macOS 通用（UTF-8）。

用法：
    python paper_search_client.py -q "causal inference difference-in-differences" --pretty
    python paper_search_client.py -q "high dimensional lasso" --mode fine --count 20 --years 5
    python paper_search_client.py -q "labor economics" --mode broad --concept-pattern "labor|wage|employment"
    python paper_search_client.py -q "regression discontinuity" --out "D:/papers" --verify
    python paper_search_client.py -q "panel data" --dry-run --json-out raw.json      # 只打印+存 JSON

退出码：
    0 成功；2 全部数据源失败；3 参数错误；4 检索成功但校验未达标（--verify）
"""
...
USER_AGENT = "se_paper_search_skill/0.1 (paper search client)"
NA = "N/A"
...
# 统计 / 计量经济 / 经济理论相关 arXiv 分类
ARXIV_CATEGORIES = (
    "econ.EM", "econ.GN", "econ.TH",
    "stat.ME", "stat.AP", "stat.ML", "stat.TH", "stat.CO", "stat.OT",
    "math.ST",
)
...
PREPRINT_HINT = re.compile(
    r"(?i)(arxiv|ssrn|npc|rccm|cowles|cep|iza|nber|working\s*paper|preprint)")
...
def urlopen_with_ssl_fallback(req: "urllib.request.Request", timeout: int):
    """urlopen 只读兜底：本机证书链验证失败时（部分 Windows 环境对
    export.arxiv.org / api.semanticscholar.org 缺中间证书）退回 unverified
    context 再试一次。先正常验证，仅在 SSL 验证失败时降级。"""
    try:
...
def http_get_json(
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            with urlopen_with_ssl_fallback(req, timeout) as resp:
...
def norm_arxiv_id(raw: Any) -> str:
    """'http://arxiv.org/abs/2309.12345v1' / 'arXiv:2309.12345' → '2309.12345v1'。"""
    if not raw:
        return ""
...
def infer_mode(query: str, explicit: Optional[str]) -> str:
    ...
    if explicit in ("fine", "broad"):
        return explicit
    if re.search(r"(?i)概览|立项|综述|摸底|survey|landscape|全景|review|handbook", query):
        return "broad"
    if re.search(r"(?i)深度|方法|创新|对比|复现|估计量|识别", query):
        return "fine"
    return "fine"
...
def parse_semanticscholar(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    r["arxiv_id"] = norm_arxiv_id(ext.get("ArXiv"))
...
def parse_arxiv_entry(entry: "ET.Element") -> Optional[Dict[str, Any]]:
    ...
    r = blank_record()
    r["title"] = title
    r["abstract"] = re.sub(r"\s+", " ", text("summary"))
    r["arxiv_id"] = norm_arxiv_id(text("id"))
    published = text("published")
    try:
        r["year"] = int(published[:4]) if published else None
    except ValueError:
        r["year"] = None
    r["authors"] = [
    ...
    cats = [c.get("term") for c in entry.findall("atom:category", _ATOM_NS) if c.get("term")]
    if prim is not None and prim.get("term"):
        r["arxiv_cats"] = [prim.get("term")] + [c for c in cats if c != prim.get("term")]
    else:
        r["arxiv_cats"] = cats
    r["keywords"] = cats[:8]
    if cats:
        r["kw_note"] = "学科分类标签，来自 arXiv categories"
    doi = text("DOI", "arxiv")
    r["doi"] = norm_doi(doi)
    jref = text("journal_ref", "arxiv")
    pdf = ""
    for link in entry.findall("atom:link", _ATOM_NS):
        if link.get("title") == "pdf":
            pdf = link.get("href") or ""
    if jref or doi:
        # 已正式发表（有 journal ref 或出版商 DOI）——不算预印本
        r["venue"] = re.sub(r"\s*,?\s*\d+\s*$", "", jref) or "（据 DOI，见出版商页）"
        r["oa_url"] = pdf
    else:
        r["venue"] = f"arXiv:{r['arxiv_cats'][0] if r['arxiv_cats'] else 'preprint'}"
        r["is_preprint"] = True
        r["preprint_server"] = f"arXiv:{r['arxiv_id']}"
        r["oa_url"] = pdf or f"https://arxiv.org/abs/{r['arxiv_id']}"
        r["pub_types"] = ["preprint"]
    r["source"] = "arxiv"
    r["sources"] = ["arxiv"]
    return r
...
def search_arxiv(
    query: str, limit: int, years_lo: Optional[int], categories: Sequence[str],
    log: List[Dict[str, Any]],
) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """arXiv Atom API：econ.EM / stat.* / math.ST 等分类内的计量与统计文献。

    arXiv 的精确短语匹配对多词 query 过严（整句必须连排出现），因此先按
    短语查，0 命中时退回逐词 AND 再查一次（仍遵守 3s 礼仪间隔）。
    """
    cat_expr = " OR ".join(f"cat:{c}" for c in categories)
    tokens = [t for t in re.findall(r"[A-Za-z][A-Za-z0-9'\-]+", query)
              if t.lower() not in STOPWORDS]
    queries = [f'all:"{query}"']
    if len(tokens) > 1:
        queries.append("(" + " AND ".join(f"all:{t}" for t in tokens[:8]) + ")")
    n = min(max(limit, 1), 100)

    def _fetch(search_query: str) -> Tuple[Optional[ET.Element], Optional[str]]:
        params = {
            "search_query": f"{search_query} AND ({cat_expr})",
            "start": "0",
            "max_results": str(n),
            "sortBy": "relevance",
        ...
        hdr = {"User-Agent": USER_AGENT, "Accept": "application/atom+xml"}
        last_err: Optional[str] = None
        for attempt in range(3):
            t0 = time.time()
            try:
                req = urllib.request.Request(url, headers=hdr)
                with urlopen_with_ssl_fallback(req, 25) as resp:
                    payload = resp.read().decode("utf-8", errors="replace")
                root = ET.fromstring(payload)
                log.append({
                    "ts": datetime.now().isoformat(timespec="seconds"),
                    "url": ARXIV_API, "status": 200,
                    "latency_ms": int((time.time() - t0) * 1000),
                })
                return root, None
            except urllib.error.HTTPError as exc:
            ...
            time.sleep(3.0 * (attempt + 1))  # arXiv 礼仪：退避不低于 3s 基准
        return None, last_err or "unknown error"
    ...
    for i, sq in enumerate(queries):
        root, err = _fetch(sq)
        if root is None:
            if i == 0 and len(queries) > 1:
                continue
            return [], err
        entries = root.findall("atom:entry", _ATOM_NS)
        if entries or i == len(queries) - 1:
            break
        time.sleep(3.0)  # arXiv 礼仪间隔后再试 fallback 查询

    out = []
    for entry in (root.findall("atom:entry", _ATOM_NS) if root is not None else []):
        rec = parse_arxiv_entry(entry)
        if not rec:
            continue
        if years_lo and rec.get("year") and rec["year"] < years_lo:
            continue
        out.append(rec)
    return out, None
...
def merge_records(records: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rec in records:
        ...
        for field in ("title", "abstract", "venue", "doi", "tldr", "oa_url",
                      "volume", "issue", "page", "pmid", "arxiv_id"):
        ...
        if rec.get("is_preprint") and not base.get("is_preprint"):
            base["is_preprint"] = True
            base["preprint_server"] = base.get("preprint_server") or rec.get("preprint_server") or ""
        if not base.get("arxiv_cats") and rec.get("arxiv_cats"):
            base["arxiv_cats"] = rec["arxiv_cats"]
...
def apply_type_filter(records: List[Dict[str, Any]], mode: str, pub_type: str) -> List[Dict[str, Any]]:
    ...
    if pub_type == "review" or mode == "broad":
        # 经济学粗放 = 综述 + 工作论文（arXiv/NBER/SSRN 预印本是该领域主要流通形态）；
        # 仅显式 --type review 时才排除预印本
        if mode == "broad":
            kept = [r for r in records if looks_like_review(r) or r.get("is_preprint")]
        else:
...
def load_journal_index() -> Dict[str, Dict[str, Any]]:
    path = DATA_DIR / "se_journal_tiers.json"
    if not path.is_file():
        return {}
    try:
        data = json.loads(path.read_text(encoding="utf-8-sig"))
...
def render_markdown(
    ...
    lines.append("")
    stats = [f"⭐ Top {top} 篇", f"Tier-1 {zone1} 篇", f"🔴 预警 {warning} 篇",
    ...
    for i, rec in enumerate(records, 1):
        ...
        if rec.get("arxiv_id"):
            ax = rec["arxiv_id"]
            lines.append(f"- **arXiv**：[arXiv:{ax}](https://arxiv.org/abs/{ax})")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="se_paper_search 检索客户端（arXiv → OpenAlex → SS → Crossref 多源合并 + 去重 + Markdown 导出）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python paper_search_client.py -q \"causal inference difference-in-differences\" --pretty\n"
            "  python paper_search_client.py -q \"lasso high dimensional\" --count 20 --years 5\n"
            "  python paper_search_client.py -q \"labor economics\" --mode broad --concept-pattern \"labor|wage|employment\"\n"
            "  python paper_search_client.py -q \"panel data\" --dry-run --json-out raw.json\n"
        ),
    )
    ...
    p.add_argument("--sources", default="arxiv,openalex,ss,crossref",
                   help="参与检索的源（逗号分隔，默认 arxiv,openalex,ss,crossref）")
    p.add_argument("--arxiv-cats", default=",".join(ARXIV_CATEGORIES),
                   help="arXiv 分类白名单（逗号分隔，默认 econ.EM/econ.GN/econ.TH/stat.*/math.ST）")
    ...
    p.add_argument("--verify", action="store_true", help="导出后调用 validate_output.py 做 DOI + arXiv ID 反查")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # 0) 领域主渠道：arXiv（econ.EM / stat.*，工作论文与预印本为主）
    cats = [c.strip() for c in (args.arxiv_cats or "").split(",") if c.strip()] or list(ARXIV_CATEGORIES)
    def _why(e: Optional[str]) -> str:
        return e or "连接正常但 0 命中（arXiv/OpenAlex/SS 检索仅支持英文，中文查询请改用英文关键词）"

    if "arxiv" in srcs:
        got, err = search_arxiv(query, max(count * 2, 20), years_lo, cats, logs)
        if got:
            records.extend(got)
            sources_used.append("arXiv")
        else:
            notes.append(f"{warn} arXiv 不可用（{_why(err)}），已跳过该源。")
            print(f"{warn} arXiv 不可用：{_why(err)}；已切下游源。", file=sys.stderr)
...
def run_verify(md_path: Path) -> int:
    """复用同目录的 validate_output.py 做 DOI + arXiv ID 反查。"""
    ...
    total = summary.get("combined_total", 0)
    ok = summary.get("combined_verified", 0)
    msg = (f"DOI {summary.get('verified', 0)}/{summary.get('total_dois', 0)} · "
           f"arXiv {summary.get('verified_arxivs', 0)}/{summary.get('total_arxivs', 0)}")
    if rate is not None and rate < 0.95:
        print(f"❌ 反查通过率 {rate * 100:.1f}% < 95%：{ok}/{total}（{msg}）")
        return 2
    print(f"✅ 反查通过：{ok}/{total}（{msg}）")
    return 0
