...
> 注：本 skill 主渠道 arXiv 与元数据权威 Crossref **无需任何 key**（arXiv 礼仪限速 1 req/3s，Crossref 建议带 mailto）；key 只影响 OpenAlex / Semantic Scholar 两个源的限速。
...
│   └── se_paper_search_setup.ps1  ← 首次初始化
└── ...
```
...
```powershell
# 完整 API 模式
.\se_paper_search_setup.ps1 -Mode api -OpenAlexKey "l1..." -SSKey "abc..."

# 只 OpenAlex，不 SS
.\se_paper_search_setup.ps1 -Mode api -OpenAlexKey "l1..."

# 无 API 模式（最简）
.\se_paper_search_setup.ps1 -Mode noapi
```
...
- ✅ 团队成员各自 `se_paper_search_setup.ps1` 一次

## 📋 申请地址

| Provider | 申请 URL | 限流 |
|---|---|---|
| arXiv Export API | 无需 key | 礼仪限速 1 req/3s（脚本已内置间隔） |
| Crossref | 无需 key（可选 mailto 进 polite pool） | polite pool 50 req/s |
...
2. 运行 .\se_paper_search_setup.ps1 一次
3. 后续运行只用 .ps1，不重新配置
...
| `api_keys.local.json not found` | 未初始化 | 跑 `se_paper_search_setup.ps1` |
